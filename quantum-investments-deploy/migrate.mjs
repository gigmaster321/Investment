/**
 * Quantum Investments – Database Migration Runner
 *
 * Idempotent: safe to run on both a fresh database and an existing one.
 * All tables use IF NOT EXISTS guards.
 *
 * Usage:
 *   DATABASE_URL=postgresql://... node migrate.mjs
 * or with .env loaded via a tool such as dotenv-cli:
 *   npx dotenv -e .env -- node migrate.mjs
 */
import pg from "pg";
...
/* Seed the three default investment plans — idempotent, never overwrites admin edits. */
await pool.query(`
